...
const data=JSON.parse(fs.readFileSync(path.join(root,'fractions.json'),'utf8'));
...
for(const c of data.chapters){assert(c.cues.every(s=>s.text.length>80));assert.equal(c.questions.length,2);assert.equal(c.questions[0].type,'choice');assert.equal(c.questions[0].options.length,6);assert.equal(new Set(c.questions[0].options).size,6);assert.equal(c.questions[1].type,'number');}
...
vm.runInContext(fs.readFileSync(path.join(root,'fractions.js'),'utf8'),context);
...
setImmediate(()=>{
 ...
 clock=9999;el.get('next').onclick();assert.equal(vm.runInContext('cue',context),0);
 clock=10000;el.get('next').onclick();context.window.geometryVoiceEnd(firstId,true);
 ...
 el.get('answers').children[1].onclick();assert.equal(el.get('continue').hidden,true);
 ...
 el.get('continue').onclick();assert.equal(el.get('heading').textContent,'Reading fractions');
 // Visit every model and solve every remaining checkpoint through the player.
 for(let ch=1;ch<10;ch++){
  for(let i=0;i<4;i++)nextScene();
  for(const q of data.chapters[ch].questions){
 ...
 const saved=JSON.parse(storage['axognition-fractions-v1']);assert.equal(Object.keys(saved.answers).length,10);
